...
{
$freteorcadototal = $freteorcadototal + $row['Frete_Orcado'];
}
...
WHERE	
n.nfcodempresa not in (15,9551,5094)
and n.nftransportador not in (15,2136,5160,4348,4868,4368,29262,0)
and n.nftipofrete in ('EMITENTE')
and n.nfemissao between '".$ano."-".$mes."-01' and '".$ano."-".$mes."-31'
and i.nficfop not in (5910,6910,5915,6915)
and i.nfipedido = 0
...
// Quantidade de Envios via Pamavi ( sem Viaje_Seguro, sem cortesias e sem consertos )
$sql = "SELECT 
n.nfnumero as Nota,
...
WHERE	
n.nfcodempresa not in (15,9551,5094)
and n.nftransportador in (4348,4868)
and n.nftipofrete in ('EMITENTE')
and n.nfemissao between '".$ano."-".$mes."-01' and '".$ano."-".$mes."-31'
and i.nficfop not in (5910,6910,5915,6915)
group by Nota desc";
...
{
$qtdpamavi++;
$freteorcadopamavi = $freteorcadopamavi + $row['Frete_Orcado'];
}	

/*
// Quantidade de Envios via Viaje_Seguro ( sem cortesias e sem consertos )
$sql = "SELECT 
n.nfnumero as Nota,
...
WHERE	
n.nfcodempresa not in (15,9551,5094)
and n.nftransportador in (29262)
and n.nftipofrete in ('EMITENTE')
and n.nfemissao between '".$ano."-".$mes."-01' and '".$ano."-".$mes."-31'
and i.nficfop not in (5910,6910,5915,6915)
group by Nota desc";
...
{
$qtdviajeseguro++;
$freteorcadoviajeseguro = $freteorcadoviajeseguro + $row['Frete_Orcado'];
}
*/


// Quantidade de Envios via Serafinense( sem cortesias e sem consertos )
$sql = "SELECT 
n.nfnumero as Nota,
...
WHERE	
n.nfcodempresa not in (15,9551,5094)
and n.nftransportador in (15,5160,0)
and n.nftipofrete in ('EMITENTE')
and n.nfemissao between '".$ano."-".$mes."-01' and '".$ano."-".$mes."-31'
and i.nficfop not in (5910,6910,5915,6915)
group by Nota desc";
...
{
$qtdinterno++;
$freteorcadointerno = $freteorcadointerno + $row['Frete_Orcado'];
}		
...
/*
// Quantidade de Envios via Correios ( Sem Serafinense - Sem Pamavi, sem Viaje_Seguro, sem cortesias e sem consertos )
$sql = "SELECT 
n.nfnumero as Nota,
...
while ($row = mysqli_fetch_array($query))
{
$correios = $correios + $row['Frete'];
$qtdcorreios++;
$freteorcadocorreios = $freteorcadocorreios + $row['Frete_Orcado'];
}
...
<html>
  ...
  <body>
    ...
    <hr>
    <h3> Frete Orçado: <?php echo " R$ ".number_format($freteorcadototal,2,',','');?></h3>
    <hr>
    ...
    <ul type="square"> 
      ...
      <li><?php echo $qtdpamavi;?> Envios Via Favero: <?php echo "R$ ".number_format($pamavi,2,',','');?> <u><strong><font size=3><?php echo "    |Orçado: R$ ".number_format($freteorcadopamavi,2,',','')." ";?> </font></strong></u> </li>
	  <!-- Não há necessidade de incluir estes no momento
	  <li><?php echo $qtdviajeseguro;?> Envios Via Viaje Seguro: <?php echo "R$ ".number_format($viaje_seguro,2,',','');?> <u><strong><font size=3><?php echo "    |Orçado: R$ ".number_format($freteorcadoviajeseguro,2,',','')." ";?> </font></strong></u> </li>
	  <li><?php echo $qtdcorreios;?> Envios Via Correios: <?php echo "R$ ".number_format($correios+$envios,2,',','');?> <u><strong><font size=3><?php echo "    |Orçado: R$ ".number_format($freteorcadocorreios,2,',','')." ";?> </font></strong></u> </li>
		-->
	</ul>
    <strong>&emsp;Total Despesas: </strong> <?php echo "R$ ".number_format($totalgastoexterno,2,',','');?>
    <hr>
    <h3> Logistica Interna:    </h3> 
    <ul type="square"> 
      <li><strong><?php echo $qtdinterno;?> Envios de Materiais </strong>
        <!--<ul type="circle"> 
          <li>Combustivel: <?php echo "R$ ".number_format($combustivel,2,',','');?></li>
          <li>Manutenções: <?php echo "R$ ".number_format($manutencao,2,',','');?></li>
          <li>Diversos <font size=2>(Alimentação/Gastos em Dinheiro Vivo) </font>: <?php echo "R$ ".number_format($diversos,2,',','');?></li>
        </ul>
		-->
    </ul>
  <!--  &emsp;<strong>Total Despesas: </strong> <?php echo "R$ ".number_format($combustivel+$manutencao+$diversos,2,',','');?>  -->
    <u><strong><font size=3><?php echo "    |Orçado: R$ ".number_format($freteorcadointerno,2,',','')." ";?> </font></strong></u>
    ...
    <ul>
     ...
     <!-- <li>Envios/Coletas de Motoboy: <?php echo "R$ ".number_format($motoboy,2,',','');?></li>  -->
    ...
    <ul type="square"> 
      <li><strong>Total Frete Orçado: <?php echo "R$ ".number_format($freteorcadototal,2,',','');?></strong></li>
	<hr align=left width=400>   
	    <ul>
        <li> - Logistica Interna ( Abatendo ): <?php echo "R$ ".number_format($freteorcadointerno,2,',','');?><!--<u> <font size=3><?php// echo "    |Orçado: R$ ".number_format($freteorcadotransportadores,2,',','')." ";?> </font></u></li>  !-->
		</ul>
		
	 <strong>&emsp;Total Parcial: </strong> <?php echo "R$ ".number_format($freteorcadototal-($freteorcadointerno),2,',','');?>
     ...
     <hr align=left width=400>
      ...
      <ul>
        ...
        <li> - Favero: <?php echo "R$ ".number_format($pamavi,2,',','');?><!--<u> <font size=3><?php //echo "    |Orçado: R$ ".number_format($freteorcadopamavi,2,',','')." ";?> </font></u></li> !-->
		<!-- <li> - Viaje Seguro: <?php echo "R$ ".number_format($viaje_seguro,2,',','');?><!--<u> <font size=3><?php //echo "    |Orçado: R$ ".number_format($freteorcadoviajeseguro,2,',','')." ";?> </font></u></li> !-->
	 	<!-- <li> - Correios: <?php echo "R$ ".number_format($correios+$envios,2,',','');?><!--<u> <font size=3><?php //echo "    |Orçado: R$ ".number_format($freteorcadoviajeseguro,2,',','')." ";?> </font></u></li> !-->
	
	</ul>
      <strong>&emsp;Total Parcial: </strong> <?php echo "R$ ".number_format($freteorcadototal-($freteorcadointerno+$pamavi+$correios+$envios+$transportadores+$viaje_seguro),2,',','');?>
      <hr align=left width=400>
      <!--<ul>
        <li> - Combustivel: <?php echo "R$ ".number_format($combustivel,2,',','');?></li></li>	
    <li>- Manutenções: <?php echo "R$ ".number_format($manutencao,2,',','');?></li></li>
  <li>- Diversos: <?php echo "R$ ".number_format($diversos,2,',','');?></li></li>
	
<strong>Total Parcial: </strong> <?php echo "R$ ".number_format($freteorcadototal-($freteorcadointerno+$viaje_seguro+$correios+ $pamavi+$transportadores+$combustivel+$envios+$manutencao+$diversos),2,',','');?>
<hr align=left width=400>

		-->
<li> - Envios sem Cobrança Orçada: <?php echo "R$ ".number_format(($malotes+$motoboy+$fretecortesia+$freteconsertos+$sempedido),2,',','');?></li>

<hr align=left width=400> 
<mark><strong><font size=5>&ensp;Total Geral: <?php echo "R$ ".number_format($freteorcadototal-($freteorcadointerno+$correios+$viaje_seguro + $transportadores+$pamavi+$freteconsertos+$fretecortesia+$combustivel+$envios+$manutencao+$diversos+$malotes+$sempedido+$motoboy),2,',','');?>&ensp; </font></strong> </mark>
